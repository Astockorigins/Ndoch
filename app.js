...
function applySentimentIG(longPct, shortPct, clients){
  ...
  if(dom){
    dom.textContent = (lp >= sp) ? `${lp.toFixed(0)}%` : `${sp.toFixed(0)}%`;
  }
  if(domLbl){
    domLbl.textContent = (lp >= sp) ? 'LONG' : 'SHORT';
  }
  ...
  if(clientsEl){
    clientsEl.textContent = (clients != null && clients !== '') ? String(clients) : '—';
  }
}

// Fallback mirror (keeps working even if sentiment values are set elsewhere)
