(function () {
  try {
    ...
    const ui = {
      ...
      // OPTIONAL: Gold Strength Chart (won’t crash if missing)
      goldChart: $("goldChart"),
      chartPill: $("chartPill"),
    ...
    function updateSentimentUI(ok) {
      ...
      if (ok) {
        ...
        const clientsVal =
          sentiment.clients !== null && sentiment.clients !== undefined && sentiment.clients !== "" ? String(sentiment.clients) : "—";
        if (ui.sentClients) ui.sentClients.textContent = clientsVal;

        try {
          applySentimentIG(longPct, shortPct, clientsVal);
        } catch (e) {}
    ...
    function applyTheme(mode) {
      ...
      if (ui.themeBtn) ui.themeBtn.textContent = themeMode === "light" ? "🌙 Dark" : "☀️ Light";

      const meta = ui.themeColorMeta;
      if (meta) meta.setAttribute("content", themeMode === "light" ? "#F8FAFC" : "#0B0F19");
    }
    ...
    function computeYesterdayHighLow(bars) {
      const map = new Map();
      for (const b of bars) {
        const d = parseISOish(b.datetime);
        if (!d) continue;
        const k = dayKeyTZ(d);
    ...
    // =========================
    // GOLD STRENGTH CHART (REAL M15) - SAFE
    // =========================
    ...
    async function refresh() {
      ...
      try {
        ...
        // optional soft alert
        softAlert(decision, reason, score, loc.loc, isNewsLockedNow());
      } catch (err) {
        console.error(err);
        setConn(false);
    ...
    on(ui.threshold, "change", () => {
      save();
      refresh();
    });
    on(ui.levelDistance, "change", () => {
      save();
      refresh();
    });
    on(ui.levelsLookback, "change", () => {
      save();
      refresh();
    });
    on(ui.autoRefresh, "change", () => {
      setAuto();
      refresh();
    });
    ...
    on(ui.newsPre, "change", () => {
      news.pre = parseInt(ui.newsPre.value || "60", 10);
      updateNewsUI();
      save();
    });
    on(ui.newsPost, "change", () => {
      news.post = parseInt(ui.newsPost.value || "60", 10);
      updateNewsUI();
      save();
    });
    ...
    window.addEventListener("resize", () => {
      try {
        renderGoldStrengthFromM15(window.__LAST_M15 || []);
      } catch (e) {}
    });
...
function applySentimentIG(longPct, shortPct, clients) {
  ...
  if (donut) {
    donut.style.background = `conic-gradient(#60A5FA 0deg, #60A5FA ${deg}deg, #F87171 ${deg}deg, #F87171 360deg)`;
  }
...
function applySentimentIGFallback() {
  try {
    ...
    if (isFinite(lp) && isFinite(sp)) applySentimentIG(lp, sp, c);
  } catch (e) {}
}
